...
* `aria-controls`: A data attribute containing the CSS selector of the element to hide or show. This should be defined in the HTML by the developer.
* `data-element-hidden`: A data attribute generated by JavaScript that indicates whether the element referenced by `aria-controls` is hidden. By default, it is considered `false`, indicating that the element is visible. This attribute can also be used to determine the control's content (e.g., button label).
